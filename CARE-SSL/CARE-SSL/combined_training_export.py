...
RTSP_COUNTER_SCRIPT = (
    ...
    / "199 occ count code with masking"
    / "rtsp_zone_tracker_updated2.py"
)
RTSP_MASK_DIR = RTSP_COUNTER_SCRIPT.parent / "masks"
RTSP_CAM_SOURCES = {
    "cam1": os.getenv(
        "RTSP_CAM1_SOURCE",
...
def load_rtsp_counter_module():
    module_name = "rtsp_zone_tracker_updated2_embedded"
    if module_name in sys.modules:
        return sys.modules[module_name]
    if not RTSP_COUNTER_SCRIPT.exists():
        raise FileNotFoundError(f"RTSP counter script not found: {RTSP_COUNTER_SCRIPT}")

    spec = importlib.util.spec_from_file_location(module_name, RTSP_COUNTER_SCRIPT)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Could not load RTSP counter script: {RTSP_COUNTER_SCRIPT}")
    ...
    try:
        ...
        raise RuntimeError("RTSP counter dependencies are not installed.") from exc
    except Exception:
        sys.modules.pop(module_name, None)
        raise
    return module
...
class RtspPeopleCounter:
    ...
    def start(self) -> None:
        try:
            self.rtsp = load_rtsp_counter_module()
            self.device = self.rtsp.resolve_device(self.device_value)
            cam1 = self.rtsp.CameraState(
                name="cam1",
                source=self.cam1_source,
                conf=self.conf1,
                imgsz=self.imgsz,
            )
            cam2 = self.rtsp.CameraState(
                name="cam2",
            ...
            for cam in self.cams:
                cam.cap = self.rtsp.cv2.VideoCapture(cam.source)
            ...
            cam1.mask_bin = self.rtsp.load_mask_binary(
                self.mask1_path,
                first_frames["cam1"].shape[:2],
            )
            cam2.mask_bin = self.rtsp.load_mask_binary(
                self.mask2_path,
            ...
            for cam in self.cams:
                thread = threading.Thread(
                    target=self.rtsp.camera_capture_loop,
                    args=(cam,),
                    daemon=True,
                )
                thread.start()
            ...
            self.model = self.rtsp.YOLO(self.model_path)
                    ...
                    if len(results) > 0:
                        ...
                        if hasattr(result, "boxes") and result.boxes is not None:
                            ...
                            if xyxy is not None:
                                ...
                                count = self.rtsp.center_in_mask_count(
                                    cam.mask_bin,
                                    boxes,
                                    overlap_thresh=0.0,
                                )
