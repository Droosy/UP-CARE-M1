...
Notes:
 ...
 - Also writes the latest combined count to a small JSON "bridge" file every
   OUTPUT_INTERVAL seconds, so other scripts (e.g. code.py) can pick it up.
...
# ==================== PERSON COUNT BRIDGE ====================
# Path to the JSON file this script writes on every SUMMARY output. Must
# match PERSON_COUNT_FILE in code.py exactly so it can read the same file.
    ...
    try:
        while True:
            ...
            if now - last_output_time >= OUTPUT_INTERVAL:
                ...
                # Write latest count to the bridge file for code.py to pick up
                write_person_count_file(PERSON_COUNT_FILE, ts, out_cam1, out_cam2, combined)
