...
def parse_args():
    ...
    p.add_argument("--frame-interval", type=int, default=125,
...
def camera_report(cam: CameraState, stale_timeout: float, stale_behavior: str):
    """
    Decide what count to report for a camera, based on how fresh its last
    detection actually is.

    Returns (count, is_stale, age_seconds).
    ...
    and by default contributes 0 rather than propping the occupancy count up
    indefinitely.
    """
    age = cam.seconds_since_detection()
    is_stale = age > stale_timeout
    if not is_stale:
        return int(cam.last_frame_counts or 0), False, age
    if stale_behavior in ("hold", "last"):
        return int(cam.last_frame_counts or 0), True, age
    return 0, True, age
...
def run_live_mode(cam1, cam2, model, device, args, writer, csv_writer, csv_file, output_interval, h1, w1, h2, w2):
    ...
    while True:
        ...
        if now - last_output_time >= output_interval:
            # Freshness-aware reporting. A camera's count is only reported as a
            # real occupancy value if a detection actually ran recently. If the
            # stream died, we do NOT keep echoing the last number we ever saw.
            out_cam1, stale1, age1 = camera_report(cam1, args.stale_timeout, args.stale_behavior)
            out_cam2, stale2, age2 = camera_report(cam2, args.stale_timeout, args.stale_behavior)
            ...
            print(f"[{ts}] SUMMARY (every {int(output_interval)}s): "
                  f"cam1={out_cam1}{tag1}  cam2={out_cam2}{tag2}  "
